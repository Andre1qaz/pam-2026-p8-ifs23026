...
class TodoService {
  ...
  Future<ApiResponse<TodoPaginatedModel>> getTodos({
    ...
    if (response.statusCode == 200) {
      ...
      // Support both paginated (with meta) and non-paginated backend
      int total = list.length;
      int totalPages = 1;
      int returnedPage = page;
      int returnedPerPage = perPage;

      if (data.containsKey('meta')) {
        final meta = data['meta'] as Map<String, dynamic>;
        total      = meta['total']      as int? ?? list.length;
        totalPages = meta['totalPages'] as int? ?? 1;
        returnedPage    = meta['page']    as int? ?? page;
        returnedPerPage = meta['perPage'] as int? ?? perPage;
      }

      return ApiResponse(
        success: true,
        message: body['message'] as String,
        data: TodoPaginatedModel(
          todos:      list,
          page:       returnedPage,
